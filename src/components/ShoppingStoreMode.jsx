...
export default function ShoppingStoreMode({ list, onToggle, onClose, categories = DEFAULT_CATEGORIES }) {
      ...
      <div className="smode-body">
        {allDone && (
          <div className="smode-all-done">
            ...
            {hasPrices && (
              <p className="smode-all-done-total">Total spent: <strong>${runningTotal.toFixed(2)}</strong></p>
            )}
            ...
            <div key={cat.id} className="smode-cat">
              ...
              <div className="smode-items">
                ...
                {catItems.filter(i => i.checked).map(item => (
                  <button
                    key={item.id}
                    className="smode-item smode-item--done"
                    onClick={() => onToggle(item.id)}
                  >
                    <div className="smode-checkbox smode-checkbox--checked">
                      <IconCheck />
                    </div>
                    <div className="smode-item-body">
                      <span className="smode-item-name">{item.name}</span>
