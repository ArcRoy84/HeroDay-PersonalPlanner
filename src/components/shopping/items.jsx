...
function ItemCard({ item, cat, purchase, stores, lastPrice, onConfirmPrice, onToggle, onUpdate, onEdit, onDelete }) {
  const adjQty = (delta) =>
    ...
    <div className={`sic ${item.checked ? 'sic--done' : ''}`} style={{ '--cc': cat.color }}>
      ...
      <div className="sic-info">
        ...
        {item.checked && purchase && !purchase.confirmed && onConfirmPrice && (
          <PriceEditor item={item} purchase={purchase} stores={stores ?? []} lastPrice={lastPrice} onConfirm={onConfirmPrice} />
        )}
      </div>
...
function CategorySection({
  cat, items, purchaseByItem, stores, lastPriceForItem, onConfirmPrice, onToggle, onUpdate, onEdit, onDelete,
}) {
        ...
        <div className="sic-list">
          {unchecked.map(item => (
            <ItemCard key={item.id} item={item} cat={cat}
              purchase={purchaseByItem?.get(item.id)} stores={stores}
