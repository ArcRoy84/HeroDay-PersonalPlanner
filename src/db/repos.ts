...
/**
 * Deleting a list must also delete its items, or they are orphaned in a table
 * nothing queries. Both happen in one transaction so a failure leaves neither.
 */
export async function removeListWithItems(listId: string): Promise<void> {
  await db.transaction('rw', db.shoppingLists, db.shoppingItems, async () => {
    ...
    await db.shoppingItems.bulkPut(
      items.map(item => ({ ...item, deletedAt: timestamp, updatedAt: timestamp })),
    );
...
export async function recordPurchase(item: ShoppingItem): Promise<void> {
  const key = item.name.toLowerCase();
  const existing = await db.shoppingHistory.get(key);
  const timestamp = now();
